...
public class AutoGrader {
	...
	// Test if the code correctly implements the Runnable interface
	public boolean testRunnableInterface(String filePath) throws IOException {
		System.out.println("Starting testRunnableInterface with file: " + filePath);
		...
		// Flags to check for Runnable interface usage
		boolean hasMainMethod = false;
		boolean implementsRunnableInterface = false;
		...
		for (ClassOrInterfaceDeclaration classDecl : cu.findAll(ClassOrInterfaceDeclaration.class)) {
			if (classDecl.getNameAsString().equals("MyRunnable")) {
				implementsRunnableInterface = true;
			}
		}
		...
		System.out.println("Class 'MyRunnable' implements Runnable: " + (implementsRunnableInterface ? "YES" : "NO"));
		...
		boolean result = hasMainMethod && implementsRunnableInterface && startsThread1 && startsThread2;
